...
void CSystemOfLinearEquations::enterCoefficients(double** array) {
	for (int i = 0; i < this->countOfUnknown; i++) {
		for (int j = 0; j < countOfUnknown + 1; j++) {
			this->matrixOfCoefficients[i][j] = array[i][j];
		}
	}
}